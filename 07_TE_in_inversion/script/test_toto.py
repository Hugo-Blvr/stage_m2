...
def make_visualization(all_result, output_path):
    ...
    for iso, methode_dict in isolat_data.items(): 
        create_density_plot(methode_dict, f'Densité moyenne des TEs autour des inversions - Isolat {iso}', 
                            os.path.join(output_path, f'test_SF_te_density_isolat_{iso}.png'))
...
def visualize_te_inversion_relative_density_parallel(datas, te_data, chr_size_dict, output_dir, 
                                    inv_min_size=0, inv_max_size=1000000, flanking_region_size=2000000, step_size=5000,
                                    num_processes=None):
    """
